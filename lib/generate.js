...
module.exports = function (hToken, options) {
  ...
  return function (jwtPayload, jwtOptions) {
    ...
    var tokenDbEntryData = {
      ...
      // use a special field for jti (instead of the mongodb default `_id`),
      // so that we can generate random unguessable values for it
      // 
      // the tradeoff here is that we must check if the save errors due
      // to conflict on jti
      jti: uuid.v4(),
    };
    ...
    return tokenEntry.save()
      .then((createdTokenEntry) => {
        ...
        _signOptions.jwtid = createdTokenEntry.jti;
