...
module.exports = function (hToken, options) {
  ...
  return function (token) {
    ...
    return _jwtVerify(token, SECRET, verifyOptions)
      ...
      .catch((err) => {
        ...
        if (err.name === 'JsonWebTokenError') {
          ...
          if (errMsg === 'jwt malformed') {
            return Promise.reject(new InvalidTokenError('MalformedJWT'));
          } else if (errMsg === 'invalid signature') {
            return Promise.reject(new InvalidTokenError('InvalidSignature'));
          } else if (errMsg.startsWith('jwt issuer')) {
            return Promise.reject(new InvalidTokenError('InvalidIssuer', errMsg));
          }
      ...
      .then((blacklistEntry) => {
        if (blacklistEntry) {
          return Promise.reject(new InvalidTokenError('The token is blacklisted'));
        } else {
          return _decoded;
        }
      });    
